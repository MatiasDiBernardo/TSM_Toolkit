...
import math
from sine_model.dftModel import dftAnal
from sine_model.utilFunctions import peakDetection, peakInterp, genSpecSines
...
def sineModel(x, fs, w, N, t):
    ...
    while pin < pend:  # while input sound pointer is within sound
        ...
        # -----synthesis-----
        Y = genSpecSines(ipfreq, ipmag, ipphase, Ns, fs)  # generate sines in the spectrum
...
def sineModelSynth(tfreq, tmag, tphase, N, H, fs):
    ...
    for l in range(L):  # iterate over all frames
        ...
        Y = genSpecSines(tfreq[l, :], tmag[l, :], ytphase, N, fs)  # generate sines in the spectrum
